...
| 引数 | `--topic {keyword}`（トピック指定）、`--category {cat}`（カテゴリ絞り込み）、`--list`（履歴一覧）、`--review {id}`（復習）、`--reset`（リセット） |
...
- **優先度**: カテゴリ輪番方式（news→engineering→claude-code-docs→research）で偏りなく学習。`--category` で特定カテゴリに絞り込み可能。新しいトピックがない場合はフォールバックトピックを提示
...
## 概要
- 記事を読まなくても内容を把握できるレベルの要約（5-10文）
- 何が発表されたか、何が変わるか、なぜ重要かを含む
