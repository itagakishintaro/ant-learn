---
name: ant-learn
description: Anthropic公式サイト(https://www.anthropic.com/)の最新情報を学習するスキル。トピックの背景・詳細・お客様説明ポイントを含む学習コンテンツを生成する。実行するたびに新しいトピックを1つ提示し、学習済みのものは重複しない。`/ant-learn` で新規学習、`/ant-learn --list` で履歴一覧、`/ant-learn --review {id}` で復習、`/ant-learn --reset` で履歴リセット。
argument-hint: [--list] [--review <id>] [--reset]
---

## あなたのタスク

Anthropic公式サイトから最新トピックを1つ抽出し、SAがお客様に説明できるレベルの学習コンテンツを生成してください。
...
WebFetchツールで以下の3ページを取得し、記事のリンクとタイトルを抽出する:
...
> **カテゴリ**: {news/research/engineering}
> **公開日**: {日付}
> **URL**: {記事URL}
...
1. トピックIDを記事URLのパスから生成する（例: `/news/claude-opus-4-6` → `claude-opus-4-6`）
...
```json
{
  ...
  "title": "{トピック名}",
  "url": "https://www.anthropic.com{記事パス}",
  "category": "{news|research|engineering}",
...
公式サイトの全記事が学習済みの場合に使用する。これらのトピックについては、WebFetchで `https://www.anthropic.com` 配下の関連ページを検索して情報を収集し、学習コンテンツを生成すること。
...
- 情報源は `https://www.anthropic.com/` 配下のコンテンツのみに限定すること（外部サイトは参照しない）
- 学習コンテンツは日本語で生成すること
