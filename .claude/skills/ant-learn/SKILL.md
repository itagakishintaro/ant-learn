...
argument-hint: [--topic <keyword>] [--category <cat>] [--list] [--review <id>] [--reset]
---
...
### `--category {cat}` の場合
特定のカテゴリに絞って学習する。`{cat}` は以下のいずれか:
- `news` — Anthropicニュース・発表（https://www.anthropic.com/news）
- `research` — リサーチ論文（https://www.anthropic.com/research）
- `engineering` — エンジニアリングブログ（https://www.anthropic.com/engineering）
- `claude-code-docs` — Claude Code公式ドキュメント（https://code.claude.com/docs/ja）

指定されたカテゴリの情報源のみからトピックを取得し、「新規学習フロー」に従う（Step 2で該当カテゴリの1ページだけを取得する）。
...
4. 該当カテゴリで全て学習済みの場合: フォールバックトピックリストから同カテゴリのものを選択する

#### `--category` 指定なしの場合（カテゴリ輪番方式）
特定カテゴリに偏らないよう、以下の輪番ロジックで選択する:

1. 履歴（history.json の topics）から直近の学習済みトピックのカテゴリを確認する
2. 以下の輪番順序に従い、直近のカテゴリの**次のカテゴリ**から優先的に選択する:
   - `news` → `engineering` → `claude-code-docs` → `research` → `news` → ...
3. 優先カテゴリに未学習トピックがなければ、次のカテゴリへ移る
4. 各カテゴリ内では日付の新しい順に選択する（claude-code-docsは掲載順）
5. 全カテゴリで全て学習済みの場合: フォールバックトピックリストから未学習のものを選択する

> **例**: 前回 news のトピックを学習した → 今回は engineering から選択 → engineering が全て学習済みなら claude-code-docs → それも全て学習済みなら research → ...
...
## 概要
この記事を読まなくても内容を把握できるレベルで要約すること。以下を必ず含める：
- **何が発表・公開されたのか**（具体的な製品名・機能名・研究名）
- **何が変わるのか / 何ができるようになるのか**（ユーザーや開発者への影響）
- **なぜ重要なのか**（業界的な意義やAnthropicの戦略的位置づけ）
- 5-10文程度で記述する。箇条書きと文章を組み合わせてよい
